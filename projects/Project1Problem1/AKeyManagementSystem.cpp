...
bool reader(std::string input_filename, Employee employees[], int &nEmployees) {
  /*
      ...
      Will take in a string for filename, array of Employee array, and number of
     employees passed as reference. Function will attempt to open the input
     file. If the input file does not exist, the function returns false.
      Otherwise, the function will load the key information, then return true.

      <number of employees>
      <employee 1's name>
      <number of keys employee 1 possesses> <employee 1's key1> <employee 1's
     key 2> ... <employee 2's name> <number of keys employee 2 possesses>
     <employee 2's key1> <employee 2's key 2> ...
      .....
  */
...
void writer(std::string output_filename, Employee employees[], int nEmployees) {
  ...
  outputFile << nEmployees
             << std::endl;  // adding number of employees to beginning
...
bool addKeyForEmployee(Employee employees[], int nEmployees,
                       std::string emp_name, std::string newKey) {

  int employeeIndex;

  for (int i = 0; i < nEmployees;
       i++) {  // iiterate through employees array to find employee index
  ...
  if (employees[employeeIndex].nKeysPossessed ==
      5) {  // checker to prevent exceeding 5 keys
    std::cout << "This employee already has 5 keys!" << std::endl << std::endl;
    return false;
  }

  for (int i = 0; i < employees[employeeIndex].nKeysPossessed;
       i++) {  // checker to prevent copied keys
    if (employees[employeeIndex].keys[i] == newKey) {
      std::cout << "This employee already has this key!" << std::endl
                << std::endl;
      return false;
    }
  }

  // If all checks pass, add newKey at end of array and increase key possession
  // by 1.

  employees[employeeIndex].keys[employees[employeeIndex].nKeysPossessed] =
    newKey;
...
bool returnAKey(Employee employees[], int nEmployees, std::string emp_name,
  ...
  for (int i = 0; i < employees[employeeIndex].nKeysPossessed;
    ...
    if (i == employees[employeeIndex].nKeysPossessed - 1) {
      std::cout << "This employee does not have the specified key!" << std::endl
                << std::endl;
      return false;
    }
  }

  for (int i = keyIndex; i < employees[employeeIndex].nKeysPossessed - 1;
       i++) {  // removing specified key and shifting elements
    employees[employeeIndex].keys[i] = employees[employeeIndex].keys[i + 1];
    /*
        starting at specified keyIndex in which the index will be deleted, we
       will iterate until the number of keys possessed - 1 to prevent a
       segmentation fault error. we will set to the index above.
    */
  }

  employees[employeeIndex].keys[employees[employeeIndex].nKeysPossessed - 1] =
    "";  // Assuming keys are strings, replace "" with default value if
         // necessary
  ...
  do {
    ...
    if (userInput < 1 || userInput > 7)
      std::cout << "Not a valid option. Please try again." << std::endl
           ...
           userInput > 7);  // Loop continues only for invalid input
...
void showKeysAnEmployeePossesses(std::string name, Employee employee[],
                                 int nEmployees) {
  int employeeIndex;
  bool exists = false;
  ...
  if (exists) {
    std::cout << employee[employeeIndex].name
              << " possess the following keys: ";
...
void whichEmployeePossessesSpecificKey(Employee employee[], std::string key,
                                       int nEmployees) {
